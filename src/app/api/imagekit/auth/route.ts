...
export async function GET() {
  try {
    ...
    if (!urlEndpoint || typeof urlEndpoint !== "string" || urlEndpoint.trim() === "") {
      return NextResponse.json(
        { error: "ImageKit URL endpoint not configured. Please set NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT or IMAGEKIT_URL_ENDPOINT environment variable." },
        { status: 500 }
      );
    }
